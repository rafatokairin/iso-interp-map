...
def update_image(slider_pos):
    ...
    # coloring
    for idx, isoc in enumerate(isocontours):
        # finding contours
        ...
        # painting
        cv2.drawContours(img_output, contours, -1, tuple(interp_mapped), thickness=cv2.FILLED)

    cv2.imshow('output', img_output)

grayscale = cv2.imread('input.jpg', cv2.IMREAD_GRAYSCALE)
normalized = grayscale.astype(np.float32) / 255.0

color1 = np.array([0, 255, 0]) # green (openCV)
color2 = np.array([0, 0, 255]) # red (openCV)

isocontours = np.linspace(0, 1, num_isoc) # normalized
